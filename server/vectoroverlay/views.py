...
class DataUploader_Viweset(viewsets.ModelViewSet):
     ...
     def create(self, request, *args, **kwargs):
          ...
          flipListSum = request.POST.getlist('flips[]',[])
          if(forcePlateList == None):
               forcePlateList = request.POST.get('forcePlateNames', "")
               flipListSum = request.POST.get('flips',"")
               number_of_force_plates = 1
          else:
               number_of_force_plates = len(forcePlateList)    

          flipList = {}
          for i in range(len(flipListSum)):
               flipList[i] = (flip_dict.get(int(flipListSum[i])))
          ...
          #      return Response({'content': str(e)}, status=status.HTTP_405_METHOD_NOT_ALLOWED, headers=headers)